...
function assessPerformance() {
	...
	jsPsych.data.addDataToLastTrial({ "coins_collected": total_score, "credit_var": credit_var, "performance_var": performance_var }) // coins_collected was added by Rani
}
...
var instructTimeThresh = 50 ///in seconds
...
// var practice_trials_num = 4 // For debugging
// var test_trials_num = 8 // For debugging
...
var post_task_block = {
	...
	questions: ['<p class = center-block-text style = "font-size: 20px">סכמ/י בקצרה בבקשה מה נתבקשת לבצע במטלה זו.</p>',
		'<p class = center-block-text style = "font-size: 20px">האם יש לך הערות כלשהן לגבי מטלה זו?</p>'],
...
var feedback_instruct_text =
	'ברוכ/ה הבא/ה למטלה זו. המטלה תארך בערך כ-25 דקות. לחצ/י <strong>enter</strong> כדי להתחיל.'
...
var instructions_block = {
	...
	pages: [
		"<div class = centerbox><p class = block-text>במטלה זו את/ה צריכ/ה לקבל החלטות בשני שלבים כדי להרוויח תגמול. התגמול שיושג יומר בסוף המטלה לבונוס כספי. בכל שלב יופיעו על המסך שתי צורות (מופשטות) על רקע צבע מסויים. תבחר/י אחת מהן על ידי מקש החץ השמאלי או מקש החץ הימני.</p><p class = block-text>ראה/י מטה 'שלב' לדוגמא עם שתי צורות על רקע צבע מסויים.</p><div class = decision-left style='background:" +
		curr_colors[0] + "; top:70%; '><img class = 'decision-stim' src= '" + curr_images[0] +
		"'></img></div><div class = decision-right style='background:" + curr_colors[0] + "; top:70%;" +
		"'><img class = 'decision-stim' src= '" + curr_images[1] + "'></img></div></div>",
		'<div class = centerbox><p class = block-text>גם השלב הראשון וגם השני ייראו בדומה לזה. לאחר שתבצע/י את הבחירה שלך בשלב הראשון, תעבור/י לאחת משתי אפשרויות של השלב השני (שלב 2א ושלב 2ב). לכל אפשרות של שלב שני יש את צבע הרקע שלה ושתי צורות מופשטות.</p><p class = block-text>בסך הכל במטלה יש שלושה "שלבים": שלב ראשון שיכול להוביל לשלב 2א או לשלב 2ב, כל אחד עם צבע רקע משלו וצורות משלו.</p></div>',
		'<div class = centerbox><p class = block-text>כל אפשרות בחירה בשלב הראשון מקושרת בעיקר עם אחת מאפשרויות השלב השני. זה אומר שכל אחת מאפשרויות הבחירה בשלב הראשון תוביל אותך בסבירות גבוהה יותר לאחת האפשרויות של השלב השני ולא לאפשרות האחרת.</p><p class = block-text>למשל, צורת אחת של השלב הראשון עשויה להוביל אותך ל-2א רוב הזמן, ורק לעיתים רחוקות להביא אותך ל-2ב, בעוד הצורה השניה של השלב הראשון תעשה בדיוק ההפך.</p><p class = block-text>לאחר המעבר לאחד השלבים השניים, יש להגיב שוב על ידי לחיצה על אחד ממקשי החיצים.</p><p class = block-text>לאחר שתסיימ/י הוראות אלו תראה/י סיבוב לדוגמא. היה/י מוכנ/ה לבחור עם אחד החצים.</p></div>'
	...
	loop_function: function (data) {
		...
		if (sumInstructTime <= instructTimeThresh * 1000) {
			feedback_instruct_text =
				'עברת על ההוראות מהר מידי. בבקשה קח/י את הזמן וודא/י שאת/ה מבינ/ה את ההוראות. לחצ/י <strong>enter</strong> כדי להמשיך.'
			// 'Read through instructions too quickly.  Please take your time and make sure you understand the instructions.  Press <strong>enter</strong> to continue.'
			return true
		} else if (sumInstructTime > instructTimeThresh * 1000) {
			feedback_instruct_text = 'סיימת את ההוראות. לחצ/י <strong>enter</strong> כדי להשמיך.'
...
var second_instructions_block = {
	...
	pages: [
		'<div class = centerbox><p class = block-text>כפי שראית, את/ה מקבל/ת פידבק אחרי הבחירה שלך בשלב השני: מטבע זהב או "0" באדום. מטבעות הזהב קובעים את הבונוס הכספי שלך, אז נסה/י להשיג כמה שאפשר!</p><p class = block-text>כפי שהוזכר, יש ארבע צורות של השלב השני: שתי צורות ב-2א ושתי צורות ב-2ב. לכל אחת מארבע הצורות האלו יש סיכוי שונה להניב מטבע זהב. כדאי לך ללמוד איזו צורה היא הטובה ביותר כך שתוכל/י להרוויח כמה שיותר מטבעות.</p></div>',
		'<div class = centerbox><p class = block-text>הסיכוי להרוויח מטבע מכל צורה מהשלב השני משתנה לאורך הניסוי, כך שהבחירה הטובה ביותר בתחילת המטלה לא בהכרח תהיה הבחירה הכי טובה מאוחר יותר.</p><p class = block-text>לעומת זאת, הסיכוי לעבור לכל אחד מהשלבים השניים לאחר בחירת צורה כלשהי בשלב הראשון הוא קבוע לאורך כל המטלה. אם את/ה מוצא/ת עם הזמן שאחת הצורות מהשלב הראשון מובילה אותך ל-2א ברוב הפעמים, זה יישאר כך לאורך כל המטלה.</p></div>',
		'<div class = centerbox><p class = block-text>לאחר שתסיימ/י עם ההוראות נעשה תרגול קצר.</p><p class = block-text>לאחר התרגול נראה לך שוב את ההוראות, אבל בבקשה וודא/י כבר עכשיו שאת/ה מבינ/ה אותן הכי טוב שאת/ה יכול/ה.</p></div>'
...
var end_block = {
	...
	text: '<div class = centerbox><p class = center-block-text>תודה שהשלמת את המטלה!</p><p class = center-block-text>לחצ/י <strong>enter</strong> כדי לסיים ולעבור לשאלונים.</p></div>',
	// text: '<div class = centerbox><p class = center-block-text>תודה שהשלמת את המטלה!</p><p class = center-block-text>לחצ/י <strong>enter</strong> כדי להמשיך.</p></div>',
...
var wait_block = {
	...
	text: '<div class = centerbox><p class = center-block-text>קח/י הפסקה!</p><p class = center-block-text>לחצ/י <strong>enter</strong> כדי להמשיך.</p></div>',
...
var start_practice_block = {
	...
	text: '<div class = centerbox><p class = center-block-text>מתחילים בתרגול. לחצ/י <strong>enter</strong> כדי להתחיל.</p></div>',
...
var start_test_block = {
	...
	text: "<div class = centerbox><p class = block-text>התרגול הסתיים. כעת נתחיל במטלה האמיתית עם צורות ושלבים חדשים.</p><p class = block-text>בדיוק כמו בתרגול, כל בחירה בשלב הראשון מקושרת בעיקר עם אחד השלבים השניים ולכל צורה בשלב השני יש סיכוי מסויים להרוויח מטבע. הסיכוי של כל צורת שלב שני להרוויח מטבע זהב משתנה לאורך הניסוי, כך שהצורה הטובה ביותר בתחילת המטלה לא בהכרח תהיה הצורה הטובה ביותר מאוחר יותר. בניגוד לכך, כאשר את/ה לומד/ת לאיזה שלב בדרך כלל מביאה אותך בחירה מסויימת בשלב הראשון, זה יישאר כך לאורך כל המטלה.</p><p class = block-text>המטרה שלך היא להרוויח הכי הרבה מטבעות זהב שתוכל/י. לחצ/י <strong>enter</strong> כדי להתחיל.</p></div>",
...
function createTimeline() {
	...
	return two_stage_decision_experiment;
}
