    ...
    function show_current_page() {
      if (trial.show_clickable_nav) {
        ...
        if (trial.pages.length == 1) {
          nav_html += "<button id='jspsych-instructions-next'>סיים הוראות</button>"
          // nav_html += "<button id='jspsych-instructions-next'>End Instructions</button>"
        } else {
          if (current_page == 0) {
            nav_html += "<button id='jspsych-instructions-next'>הבא</button>"
            // nav_html += "<button id='jspsych-instructions-next'>Next</button>"
          } else if (current_page == trial.pages.length - 1) {
            if (trial.allow_backward) {
              nav_html += "<button id='jspsych-instructions-back'>הקודם</button>";
